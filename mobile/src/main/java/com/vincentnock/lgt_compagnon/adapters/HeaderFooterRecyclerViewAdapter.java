...
/**
 * Created by vincent on 24/03/2015.
 */
public abstract class HeaderFooterRecyclerViewAdapter<T, V extends View> extends RecyclerViewAdapterBase<T, V> {
     ...
     */
    @Override
    public final V onCreateItemView(ViewGroup parent, int viewType) {
     ...
     */
    protected abstract V onCreateHeaderItemViewHolder(ViewGroup parent, int headerViewType);
     ...
     * @return the view holder.
     */
    protected abstract V onCreateFooterItemViewHolder(ViewGroup parent, int footerViewType);
     ...
     * @return the view holder.
     */
    protected abstract V onCreateContentItemViewHolder(ViewGroup parent, int contentViewType);
     ...
     * @param position         the position.
     */
    protected abstract void onBindHeaderItemViewHolder(ViewWrapper<V> headerViewHolder, int position);
     ...
     * @param position         the position.
     */
    protected abstract void onBindFooterItemViewHolder(ViewWrapper<V> footerViewHolder, int position);
     ...
     * @param position          the position.
     */
    protected abstract void onBindContentItemViewHolder(ViewWrapper<V> contentViewHolder, int position);

    protected boolean shouldNotifyChangesForItem(T before, T after) {
        return false;
    }

    public void setData(final List<T> data) {

        //Adding too many cards may cause crash in ObservableRecyclerView, so skip animations in that case
        if (items.size() == 0 || Math.abs(items.size() - data.size()) > 6) {
            this.items = data;
            getItemCount(); //force setting items count
            notifyDataSetChanged();
        } else {

            //Remove possible dupplicates
            Set hs = new HashSet(data);
            if (hs.size() != data.size()) {
                data.clear();
                data.addAll(hs);
            }
            ...
            for (int i = items.size() - 1; i >= 0; --i) {
                if (getLocation(data, items.get(i)) < 0) {
                    deleteEntity(i);
                }
            }
            ...
            for (int i = 0; i < data.size(); ++i) {
                T entity = data.get(i);
                int loc = getLocation(items, entity);
                if (loc < 0) {
                    addEntity(i, entity);
                } else {
                    if (shouldNotifyChangesForItem(items.get(loc), entity)) {
                        notifyContentItemChanged(loc);
                        items.set(loc, entity);
                    }
                    if (loc != i)
                        moveEntity(loc, i);
                    else
    ...
    public void addEntity(int i, T entity) {
        ...
        //reset contentItemCount with updated value
        getItemCount();
    }

    public void addEntity(T entity) {
        if (items.size() > 0)
            addEntity(items.size() - 1, entity);
        else
            addEntity(0, entity);
    }
    ...
    private int getLocation(List<T> data, T entity) {
